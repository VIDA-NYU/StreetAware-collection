#!/usr/bin/env python3
# GPU-Accelerated Mosaic Video Synchronization with Timestamp Analysis
# Enhanced version with GPU acceleration for faster processing
...
def check_gpu_availability():
    try:
        ...
        print(f"CUDA not available: {e}, using CPU")
        return False
...
def rotate_frame_gpu(frame, angle):
    """GPU-accelerated frame rotation"""
    if angle == 0:
        return frame
    
    try:
        # For GPU acceleration, we'll use cv2.cuda operations where possible
        gpu_frame = cv2.cuda_GpuMat()
        gpu_frame.upload(frame)
        
        if angle == 90:
            # Transpose + flip for 90 degree rotation
            gpu_transposed = cv2.cuda.transpose(gpu_frame)
            result = cv2.cuda.flip(gpu_transposed, 1)  # flip horizontally
            return result.download()
        elif angle == 180:
            result = cv2.cuda.flip(gpu_frame, -1)  # flip both axes
            return result.download()
        elif angle == 270:
            # Transpose + flip for 270 degree rotation
            gpu_transposed = cv2.cuda.transpose(gpu_frame)
            result = cv2.cuda.flip(gpu_transposed, 0)  # flip vertically
            return result.download()
        else:
...
def rotate_frame_cpu(frame, angle):
    """CPU fallback for frame rotation"""
    if angle == 0:
        return frame
    elif angle == 90:
...
def rotate_frame(frame, angle, use_gpu=True):
    """Unified rotation function with GPU/CPU selection"""
    if use_gpu:
        return rotate_frame_gpu(frame, angle)
    else:
        return rotate_frame_cpu(frame, angle)
...
class CameraReader:
    def __init__(self, camera_id, data_path, output_dir=None, use_gpu=True):
        ...
        # Initialize video reader with GPU acceleration if available
        ...
        # GPU frame cache for faster processing
        self.gpu_frame_cache = []
    ...
    def get_frame_from_video_segments(self, frame_id):
        ...
        for video_file in self.video_files:
            if self.current_video is None or self.current_video_index != self.video_files.index(video_file):
                ...
                # Use GPU-accelerated video decoder if available
                if self.use_gpu:
                    try:
                        # Try to use CUDA-accelerated video reading
                        self.current_video = cv2.VideoCapture(str(video_file), cv2.CAP_FFMPEG)
                        # Note: cv2.cuda video reading requires specific build
                    except:
    ...
    def cleanup(self):
        """Clean up video reader and GPU resources."""
        if self.current_video is not None:
            self.current_video.release()
        # Clear GPU cache
        self.gpu_frame_cache = []
...
def concatenate_frames_gpu(frames, axis=1):
    """
    GPU-accelerated frame concatenation for mosaic creation.
    ...
    try:
        ...
        # Concatenate on GPU (note: cv2.cuda doesn't have hconcat/vconcat)
        # Fall back to CPU for concatenation
        result = np.concatenate(frames, axis=axis)
        return result
    except Exception as e:
        # CPU fallback
        return np.concatenate(frames, axis=axis)

def build_synchronized_videos_gpu(data_path, output_dir, threshold_ms=50, max_frames=300, fps=20, rotation=0, use_gpu=True):
    """
    Build synchronized videos using GPU acceleration and enhanced timestamp analysis.
    """
    print("=" * 60)
    print("GPU-ACCELERATED SYNCHRONIZED VIDEO BUILDER")
    print("=" * 60)
    print(f"GPU Acceleration: {'ENABLED' if use_gpu else 'DISABLED'}")
    
    processing_device = get_processing_device() if use_gpu else "cpu"
    use_gpu = use_gpu and (processing_device == "gpu")
    
    resize_function = resize_frame_gpu if use_gpu else resize_frame_cpu
    ...
    with open(os.path.join(mosaic_dir, 'master_timeline_gpu.json'), 'w') as f:
        json.dump(master_timeline, f, indent=2)
    print(f"Saved GPU master timeline to {os.path.join(mosaic_dir, 'master_timeline_gpu.json')}")
    
    # Frame tracking with enhanced metadata
    frame_tracking = {
        "synchronization_info": {
            "method": "gpu_accelerated_timestamp_analysis",
            "gpu_enabled": use_gpu,
    ...
    sample_frame_rot = rotate_frame(sample_frame, rotation, use_gpu=use_gpu)
    ...
    # Try hardware-accelerated codec first
    if use_gpu:
        # Try NVENC (NVIDIA hardware encoder) if available
        fourcc = cv2.VideoWriter_fourcc(*'avc1')  # H.264
        print("Attempting to use hardware-accelerated H.264 encoder...")
    else:
        fourcc = cv2.VideoWriter_fourcc(*'avc1')
    
    mosaic_output_file = os.path.join(mosaic_dir, 'mosaic_gpu_sync.mp4')
    ...
    for camera_id in cameras:
        cam_file = os.path.join(per_camera_dir, f"{camera_id}_gpu_sync.mp4")
    ...
    for global_timestamp in master_timeline:
        ...
        # Process each camera with GPU acceleration
        for reader in camera_readers:
            ...
            if frame is not None:
                ...
                frame = rotate_frame(frame, rotation, use_gpu=use_gpu)
        ...
        cv2.putText(info_frame, f'Frame: {frame_count}', (20, 160), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        if use_gpu:
            cv2.putText(info_frame, 'GPU: ON', (20, 200), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        
        # Arrange in 3x3 grid: info + 8 cameras
        output_frames_with_info = [info_frame] + output_frames
        
        # GPU-accelerated concatenation (falls back to CPU if needed)
        row1 = concatenate_frames_gpu(output_frames_with_info[0:3], axis=1)
        row2 = concatenate_frames_gpu(output_frames_with_info[3:6], axis=1)
        row3 = concatenate_frames_gpu(output_frames_with_info[6:9], axis=1)
        mosaic_frame = concatenate_frames_gpu([row1, row2, row3], axis=0)
    ...
    print(f"\n{'=' * 60}")
    print(f"GPU-ACCELERATED SYNCHRONIZATION RESULTS")
    print(f"{'=' * 60}")
    print(f"GPU Acceleration: {'ENABLED' if use_gpu else 'DISABLED'}")
    ...
    print(f"  Master timeline: {os.path.join(mosaic_dir, 'master_timeline_gpu.json')}")
    print(f"{'=' * 60}")

def main():
    parser = argparse.ArgumentParser(
        description="GPU-accelerated video synchronization with enhanced timestamp analysis.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
USAGE:
  python sync_video_GPU.py <data_path> [OPTIONS]

GPU ACCELERATION:
  This version uses GPU acceleration for:
  - Frame resizing (CUDA)
  - Frame rotation (CUDA)
  - Video encoding (hardware encoder if available)
...
REQUIREMENTS:
  - OpenCV compiled with CUDA support (for GPU acceleration)
  - NVIDIA GPU with CUDA support
  - Falls back to CPU if GPU is not available

EXAMPLE:
  python sync_video_GPU.py ./data --threshold 50 --fps 20 --max-frames 300
        """
    )
    parser.add_argument("data_path", help="Path to data directory containing camera folders")
    parser.add_argument("--output-dir", default="synced_output_gpu", help="Output directory for videos and tracking info")
